...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  Queue_Check(&queue);
	  UART_Functions_Check(&htim2, &flags_States);
  }
  /* USER CODE END 3 */
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	if(rxTempData[0] == '\n'){
		Circular_Buffer_RxCpltCallback(&circular_buffer, &queue);
		UART_Flag_Check(&queue, &flags_States);
		UART_Parameter_Check(&queue, &flags_States);
	}
	HAL_UART_Receive_IT(&huart2, rxTempData, 1);
}
/* USER CODE END 4 */
